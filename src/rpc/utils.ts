import { JsonRpcSubscribeRequest } from "./calls";
import {
  JsonRpcObject,
  JsonRpcRequest,
  JsonRpcResponse,
...
export function isJsonRpcResponse(value: unknown): value is JsonRpcResponse {
  return isJsonRpcObject(value) && "result" in value && "id" in value;
}
...
export function isJsonRpcSubscription(
  value: unknown
): value is JsonRpcSubscription<any> {
  return (
...
export function isJsonRpcRequest(
  value: unknown
): value is JsonRpcRequest<any, any> {
  return (
    isJsonRpcObject(value) &&
    "id" in value &&
    "method" in value &&
...
export function isJsonRpcSubscribeRequest(
  value: unknown
): value is JsonRpcSubscribeRequest {
  return (
    isJsonRpcRequest(value) &&
    value.method === "subscribe" &&
    "channels" in value.params
  );
}

export function isJsonRpcUnsubscribeRequest(
  value: unknown
): value is JsonRpcSubscribeRequest {
  return (
    isJsonRpcRequest(value) &&
