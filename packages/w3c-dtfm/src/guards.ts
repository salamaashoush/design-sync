...
import type { W3CTokenRef } from "./types/w3c";
...
export function hasTokenExtensions(
  value: unknown,
): value is RequiredKeys<DesignToken, "$extensions"> {
...
/**
 * Check if a value is a W3C $ref token reference
 */
export function isTokenRef(value: unknown): value is W3CTokenRef {
  return isObject(value) && "$ref" in value && typeof value.$ref === "string";
}

/**
 * Check if a value is any kind of reference (alias or $ref)
 */
export function isReference(value: unknown): value is TokenAlias | W3CTokenRef {
  return isTokenAlias(value) || isTokenRef(value);
}

/**
 * Check if a token has $deprecated property
 */
export function isDeprecatedToken(token: unknown): boolean {
  return isObject(token) && "$deprecated" in token;
}

/**
 * Get deprecation message from a token
 */
export function getDeprecationMessage(token: unknown): string | undefined {
  if (!isDeprecatedToken(token)) {
    return undefined;
  }
  const deprecated = (token as Record<string, unknown>).$deprecated;
