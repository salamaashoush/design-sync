...
function keyArray(items, keyFn, mapFn, options = {}) {
  ...
  solidJs.onCleanup(() => dispose(prev.values()));
  return () => {
    const list = items() || [];
    list[solidJs.$TRACK];
    return solidJs.untrack(() => {
      if (!list.length) {
        ...
        if (!options.fallback)
          return [];
        const fb2 = solidJs.createRoot((dispose2) => {
  ...
  function addNewItem(list, item, i, key) {
    solidJs.createRoot((dispose2) => {
      const [getItem, setItem] = solidJs.createSignal(item);
      const save = { setItem, dispose: dispose2 };
      if (mapFn.length > 1) {
        const [index, setIndex] = solidJs.createSignal(i);
